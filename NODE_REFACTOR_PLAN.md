...
- Phase 3 진행 중: 신규 Canvas 저장은 `canvas.document.v1`의 Canonical Node/Canvas element/Runtime sidecar로 분리되고 기존 Canvas는 다음 저장 시 전환된다. Web autosave/manual save/Publish 선행 저장은 Canonical serializer를 사용한다. Draft Run은 저장된 Canvas revision을 서버에서 Legacy execution snapshot으로 고정하며 Web이 Raw graph를 보내지 않는다. Canvas 조회 응답과 서버 내부 실행 Adapter는 아직 Legacy React Flow shape를 사용한다.
- Phase 4 진행 중: production Node 31개 중 5개는 native Executor, 26개는 immutable contract를 위한 `legacy-compatibility` Executor Adapter를 사용한다. 저장된 Canonical Canvas Run은 `data.config`를 Registry에서 materialize/검증하며 `canvas_runs.py`의 수동 camelCase parameter 평탄화는 제거됐다. Raw graph 요청만 명시적인 Legacy Run Adapter를 사용한다. Google/OpenAI/xAI text, Google/OpenAI image·Character, FAL LoRA image, Google Video와 Google/OpenAI TTS live capability는 Legacy contract digest를 유지한 채 모두 `NodeRegistry.execute`와 공통 `NodeExecutionResult`로 전환됐다. Character view/bundle, LoRA weights, Omni reference, Prompt-to-Audio lineage 및 Artifact type/schema/output role은 Manifest에서 해석한다. 테스트 전용 Fixture와 Local/Composite/Human gate operation 전환은 남았다.
...
- Phase 7 진행 중: Canvas의 직접 Legacy JSON literal write, Raw graph Draft Run, Generation Canvas Inspector의 Node key별 UI 분기, Legacy Port 비교, 중앙 수동 parameter 평탄화와 `experiments.py`의 xAI 직접 dispatch 재도입을 막는 Architecture/contract test가 추가됐다. Legacy/Unknown Edge는 Load 중 삭제하지 않고 검증 오류와 read-only 호환 경로를 유지한다. Generation Canvas의 Legacy write/run payload는 제거됐고 로컬 스토리지 1회성 Import 및 외부 구버전 API 요청만 호환 계약을 사용한다. `nodeTemplates` Legacy read 목록과 나머지 중앙 실행 dispatch 제거는 남았다.
