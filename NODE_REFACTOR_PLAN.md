...
- Phase 4 진행 중: production Node 31개 중 5개는 native Executor, 26개는 immutable contract를 위한 `legacy-compatibility` Executor Adapter를 사용한다. 저장된 Canonical Canvas Run은 `data.config`를 Registry에서 materialize/검증하며 `canvas_runs.py`의 수동 camelCase parameter 평탄화는 제거됐다. Raw graph 요청만 명시적인 Legacy Run Adapter를 사용한다. xAI text capability는 Legacy contract digest를 유지한 채 `NodeRegistry.execute`와 공통 `NodeExecutionResult`로 전환됐고 Artifact type/schema/output role을 Manifest에서 해석한다. Google/OpenAI/FAL generation과 Local operation 전환은 남았다.
