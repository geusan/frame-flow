...
기존 Manifest가 `executor=legacy-compatibility`로 이미 digest에 고정된 경우 contract를 덮어쓰지 않는다. Legacy Executor 내부에서 provider/model capability별 지원을 점진적으로 등록하고, 지원된 capability는 `NodeRegistry.execute`와 공통 `NodeExecutionResult`를 사용한다. Text generation은 개별 Node key가 아니라 Google/OpenAI/xAI text capability, Config Schema와 Artifact contract로 Prompt/Script/Text 결과를 생성한다. 표준 Image generation도 Google/OpenAI image capability와 Manifest output contract로 생성한다. Character generation은 별도 capability Executor가 view Artifact들을 먼저 만들고 이를 입력 lineage로 갖는 Character bundle을 생성한다. FAL LoRA generation은 Character와 weights Artifact lineage를 유지하는 별도 Image capability Executor를 사용한다. Google Video generation은 Omni와 일반 모델 capability를 구분하며 Character/Image/Reference Video 입력 lineage를 보존한다. Google/OpenAI TTS도 speech capability와 Audio Artifact contract를 사용한다. Local FFmpeg Executor는 Node key 대신 Manifest input Port 구조로 다중 Video 편집, Video+Audio 교체와 Timeline 렌더를 구분한다. Provider별 호출은 Node 의미를 모르는 capability API interface를 사용하고 공통 result adapter가 Artifact와 lineage를 기록한다. 아직 등록되지 않은 Local/Composite/Human gate capability만 기존 실행 Adapter로 fallback한다.
