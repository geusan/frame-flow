...
export function FormatLab() {
  ...
  return (
    <Tabs value={tab} onValueChange={(value) => setTab(value as typeof tab)} className="format-layout">
      <section className="format-browser">
        <div className="format-browser-head">
          ...
          <Button variant="secondary" size="icon" type="button" onClick={() => setView("references")} aria-label="Extract a new format from references"><Plus size={15} /></Button>
        </div>
        <SearchField className="format-search" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search formats…" />
      ...
      <section className="format-detail">
        <div className="format-detail-head">
          ...
          <div className="flex items-center gap-2"><Button type="button" onClick={() => void createVariant()} disabled={creatingVariant}><WandSparkles size={13} /> {creatingVariant ? "Creating…" : "Create variant"}</Button></div>
        </div>

        <TabsList className="format-tabs">
          {(["overview", "schema", "evidence"] as const).map((key) => <TabsTrigger value={key} key={key} className={tab === key ? "active" : ""}>{key[0].toUpperCase() + key.slice(1)}</TabsTrigger>)}
          <span className="format-save-state">Created {selected.createdAt}</span>
        </TabsList>

        <TabsContent value="overview" className="format-content">
            ...
            <Card className="format-section format-properties">
              <div className="panel-header"><div><h3>Core properties</h3><p>생성 노드가 항상 사용할 수 있는 FormatCoreV1</p></div><Badge variant="primary">format.core.v1</Badge></div>
        ...
        <TabsContent value="schema" className="format-content"><pre className="schema-viewer">{JSON.stringify({ schema_version: selected.core.schema_version, core: selected.core, extensions: selected.extensions }, null, 2)}</pre></TabsContent>
        <TabsContent value="evidence" className="format-content evidence-list">
            {Object.entries(selected.evidence).length ? Object.entries(selected.evidence).map(([field, evidence], index) => <Card className="evidence-row" key={field}><span className="evidence-index">{String(index + 1).padStart(2, "0")}</span><div><strong>{field}</strong><p>Value: {JSON.stringify(evidence.manual_override ?? evidence.value)} · Confidence: {Math.round(Number(evidence.confidence ?? 0) * 100)}% · {evidence.evidence?.length ?? 0} source spans</p></div></Card>) : <p className="experiment-history-state">이 Format에는 저장된 evidence가 없습니다.</p>}
        </TabsContent>
      </section>
    </Tabs>
  );
