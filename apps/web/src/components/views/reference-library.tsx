  ...
  return (
    <div className="view-page reference-page">
      ...
      <div className="mb-3.5 flex items-center gap-2">
        <SearchField className="min-w-[250px] max-w-[380px] flex-1" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search title, creator, or tag…" />
        <NativeSelect value={rightsFilter} onChange={(event) => setRightsFilter(event.target.value)}><option value="all">All rights</option><option value="owned">Owned</option><option value="licensed">Licensed</option><option value="analysis_only">Analysis only</option></NativeSelect>
        <NativeSelect value={statusFilter} onChange={(event) => setStatusFilter(event.target.value)}><option value="all">All status</option><option value="analyzed">Analyzed</option><option value="processing">Processing</option><option value="ready">Ready</option></NativeSelect>
          ...
          <article className="reference-card" key={item.id}>
            ...
            <div className="reference-card-body">
              ...
              <div className="card-footer">
                <Badge variant={item.allow_generation_input ? "success" : "default"} className={`rights-pill ${item.rights_basis}`}><ShieldCheck size={11} /> {rightsLabel[item.rights_basis]}</Badge>
      ...
      <Dialog open={showImport} onOpenChange={setShowImport}>
        {showImport && (
          <DialogContent className="modal-card import-modal" overlayClassName="modal-backdrop">
            <div className="modal-heading">
              <div><DialogTitle asChild><h2>Add references</h2></DialogTitle><DialogDescription asChild><p>URL을 검사한 뒤 권리 범위와 가져올 자산을 선택합니다.</p></DialogDescription></div>
              <DialogClose asChild><Button variant="secondary" size="icon" type="button" aria-label="Close add references"><X size={16} /></Button></DialogClose>
            </div>
            ...
            <label className="field-label"><span>Rights basis</span><NativeSelect value={rightsBasis} onChange={(event) => setRightsBasis(event.target.value as ReferenceRecord["rights_basis"])}><option value="analysis_only">Analysis only</option><option value="owned">Owned</option><option value="licensed">Licensed</option><option value="creative_commons">Creative Commons</option><option value="unknown">Unknown</option></NativeSelect></label>
            {inspectError && <div className="inspect-error">{inspectError} · API가 꺼져 있다면 <code>make dev-api</code>를 실행하세요.</div>}
            {inspection.length > 0 && <div className="inspection-results">{inspection.map((item) => <div key={item.canonical_url}><span className="inspection-thumb"><Play size={12} /></span><span><strong>{item.title}</strong><small>{Math.round(item.duration_ms / 1000)}s · {item.width}×{item.height} · {(item.estimated_bytes / 1_000_000).toFixed(1)} MB</small></span>{item.duplicate_reference_id ? <Badge variant="warning">Duplicate</Badge> : <CheckCircle2 size={14} />}</div>)}</div>}
            ...
            <div className="modal-actions"><DialogClose asChild><Button variant="secondary" type="button">Cancel</Button></DialogClose><Button type="button" onClick={() => void (inspection.length ? importInspected() : inspect())} disabled={inspecting || importing}>{inspecting ? "Inspecting…" : importing ? "Importing media…" : inspection.length ? "Import references" : "Inspect metadata"} <span>→</span></Button></div>
          </DialogContent>
        )}
      </Dialog>
    </div>
