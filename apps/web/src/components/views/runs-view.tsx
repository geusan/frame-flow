  ...
  return (
    <div className="view-page runs-page">
      <div className="view-heading">
        <div><h2>Workflow runs</h2><p>저장된 Generation Run과 Canvas DAG Run의 실제 실행 상태를 함께 표시합니다.</p></div>
      </div>
      <div className="run-summary-grid">
        ...
        <div className="panel"><span className="summary-icon blue"><Clock3 size={16} /></span><span><small>Completed nodes</small><strong>{summary.completedNodes}</strong><em>Persisted node results</em></span></div>
        <div className="panel"><span className="summary-icon green"><CircleDollarSign size={16} /></span><span><small>Recorded cost</small><strong>${summary.spend.toFixed(2)}</strong><em>Stored experiment costs</em></span></div>
        <div className="panel"><span className="summary-icon amber"><CalendarDays size={16} /></span><span><small>Success rate</small><strong>{summary.successRate.toFixed(1)}%</strong><em>Terminal runs only</em></span></div>
      </div>
      ...
      {!error && !loading && visibleRuns.length > 0 && <section className="panel data-table-panel">
        <table className="data-table">
          ...
          <tbody>{visibleRuns.map((run) => <tr key={run.id}>
            ...
            <td><span className="tag">{run.run_type}</span></td>
