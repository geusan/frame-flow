...
class SceneRanker(Protocol):
    ...
    def rank(self, prompt: str, frames: list[SampledFrame], candidate_count: int) -> SceneSearchResult: ...
...
def search_video_scenes(
    ...
    *,
    ...
    ranker = get_scene_ranker()
    ranked = ranker.rank(prompt.strip(), frames, candidate_count)
    return SceneSearchResult(
        ranked.scenes,
        ranked.provider,
...
def get_scene_ranker() -> SceneRanker:
