...
PROVIDER_DEFINITIONS: dict[str, ProviderDefinition] = {
    ...
    "google": ProviderDefinition(
        key="google",
        label="Gemini",
        description="Google multimodal generation and reasoning models",
        fields=(
            _field("api_key", "Gemini API key", "GEMINI_API_KEY", secret=True, placeholder="AIza…", auth_methods=("api_key",)),
            _field("project_id", "Google Cloud project", "GOOGLE_CLOUD_PROJECT", placeholder="my-gcp-project", auth_methods=("vertex",)),
            _field("location", "Vertex AI location", "GOOGLE_CLOUD_LOCATION", default="us-central1", placeholder="us-central1", auth_methods=("vertex",)),
            _field("credentials_path", "Application credentials path", "GOOGLE_APPLICATION_CREDENTIALS", placeholder="/run/secrets/google-service-account.json", help_text="Must be readable by the API and Temporal worker.", auth_methods=("vertex",)),
            _field("speech_location", "Speech location", "GOOGLE_SPEECH_LOCATION", default="global", placeholder="global", auth_methods=("vertex",)),
        ...
        auth_methods=(
            ProviderAuthMethod("api_key", "API key", "Connect with a Gemini API key from Google AI Studio.", required_fields=("api_key",)),
    ...
    "veo3": ProviderDefinition(
        key="veo3",
        label="Veo 3",
        description="Google text-to-video and image-to-video generation",
        fields=(
            _field("api_key", "Veo API key", "VEO3_API_KEY", secret=True, placeholder="AIza…", auth_methods=("api_key",)),
            _field("project_id", "Google Cloud project", "VEO3_GOOGLE_CLOUD_PROJECT", placeholder="my-gcp-project", auth_methods=("vertex",)),
            _field("location", "Vertex AI location", "VEO3_GOOGLE_CLOUD_LOCATION", default="us-central1", placeholder="us-central1", auth_methods=("vertex",)),
            _field("credentials_path", "Application credentials path", "VEO3_GOOGLE_APPLICATION_CREDENTIALS", placeholder="/run/secrets/google-service-account.json", auth_methods=("vertex",)),
            _field("output_gcs_uri", "Output GCS URI", "VEO3_OUTPUT_GCS_URI", placeholder="gs://bucket/path", auth_methods=("vertex",)),
        ),
        auth_methods=(
            ProviderAuthMethod("api_key", "API key", "Use a Gemini API key with Veo access.", required_fields=("api_key",)),
            ProviderAuthMethod("vertex", "Vertex AI", "Use a Google Cloud project with Veo enabled.", kind="cloud", required_fields=("project_id",)),
        ),
        default_auth_method="api_key",
        order=2,
    ),
